...
class StateEstimator(sm.SM):
    ...
    def __init__(self, model, verbose = False):
        ...
        :param verbose: if ``True``, prints out intermediate values
        """
        self.model = model
    ...
    def getNextValues(self, state, inp):
        ...
        :param inp: A pair ``(o, i)`` of the observation (output) and input 
...
class StateEstimatorTriggered(StateEstimator):
    """
    Like ``StateEstimator``, but the inputs are ``(observation, action,
    trigger)``.   If ``trigger`` is ``True`` then do the state update,
...
def makeStateEstimationSimulation(worldSM, verbose = False):
    ...
    :param worldSM: an instance of ``ssm.StochasticSM``
